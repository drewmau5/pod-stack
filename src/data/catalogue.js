...
export const curatedSources = [
 {id:'crime-junkie',title:'Crime Junkie',author:'audiochuck',feedUrl:'https://feeds.megaphone.fm/ADL9840290619',appleUrl:'https://podcasts.apple.com/ca/podcast/crime-junkie/id1322200189',interests:['True Crime'],format:['Standalone episodes','Investigative'],tone:['Serious']},
 {id:'dateline',title:'Dateline NBC',author:'NBC News',feedUrl:'https://podcastfeeds.nbcnews.com/dateline-nbc',appleUrl:'https://podcasts.apple.com/ca/podcast/dateline-nbc/id1464919521',interests:['True Crime','News'],format:['Investigative'],tone:['Serious','Informative']},
 {id:'daily',title:'The Daily',author:'The New York Times',feedUrl:'https://feeds.simplecast.com/54nAGcIl',appleUrl:'https://podcasts.apple.com/ca/podcast/the-daily/id1200361736',interests:['News','Politics'],format:['Interviews'],tone:['Informative']},
 ...
 {id:'criminal',title:'Criminal',author:'Vox Media Podcast Network',feedUrl:'https://feeds.megaphone.fm/VMP7924981569',appleUrl:'https://podcasts.apple.com/ca/podcast/criminal/id809264944',interests:['True Crime','Storytelling'],format:['Investigative','Narrated stories'],tone:['Serious']},
 {id:'rest-is-history',title:'The Rest Is History',author:'Goalhanger',feedUrl:'https://feeds.megaphone.fm/GLT4787413333',appleUrl:'https://podcasts.apple.com/ca/podcast/the-rest-is-history/id1537788786',interests:['History'],format:['Conversational'],tone:['Thoughtful']},
]
...
export function normalizeEpisode(ep, source, preferences={}) {
 ...
 return {...ep,sourceType:'rss',podcastId:String(source.id),feedUrl:ep.feedUrl||source.feedUrl,guid:ep.guid||ep.id,podcastName,podcastAuthor:source.author||ep.podcastAuthor||'',artworkUrl:ep.artworkUrl||source.artworkUrl,episodeArtworkUrl:ep.episodeArtworkUrl||null,webpageUrl:ep.webpageUrl||null,appleUrl:ep.appleUrl||source.appleUrl||null,spotifyUrl:ep.spotifyUrl||source.spotifyUrl||`https://open.spotify.com/search/${searchName}`,spotifyUrlType:ep.spotifyUrl||source.spotifyUrl?'direct':'search',youtubeMusicUrl:ep.youtubeMusicUrl||source.youtubeMusicUrl||`https://music.youtube.com/search?q=${searchName}`,youtubeMusicUrlType:ep.youtubeMusicUrl||source.youtubeMusicUrl?'direct':'search',interests:source.interests||source.genres||[],format:source.format||[],tone:source.tone||[],depth:source.depth||[],isArchive:age>30,isSerialized:Boolean(ep.seasonNumber||ep.episodeNumber&&ep.episodeType!=='full'),episodeNumber:ep.episodeNumber||null,seasonNumber:ep.seasonNumber||null,selectedPodcast:Boolean(preferences.selectedIds?.has(String(source.id)))}
}
...
export const fallbackSampleEpisodes=sampleEpisodes.map(x=>({...x,sourceType:'sample-fallback',feedUrl:null,guid:x.id,podcastId:`sample-${x.podcastName}`,webpageUrl:x.externalUrl,format:[x.format].filter(Boolean),tone:[x.tone].filter(Boolean),depth:[x.depth].filter(Boolean)}))
