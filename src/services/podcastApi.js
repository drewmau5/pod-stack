...
import { normalizeArtworkFields, PODSTACK_ARTWORK_FALLBACK } from '../utils/artwork.js'
...
export async function searchPodcasts(term) {
  ...
  return (data.podcasts||[]).map(show=>normalizeArtworkFields({...show,source:'apple-search'},{kind:'show'}))
}
...
].map(x=>normalizeArtworkFields({...x,podcastId:x.id,appleCollectionId:x.id,chartCountry:'ca',displayArtworkUrl:PODSTACK_ARTWORK_FALLBACK},{kind:'show'}))
export async function fetchPopularPodcasts({country='ca',limit=10}={}) {
  try { const data=await request(`/api/podcasts/charts?country=${encodeURIComponent(country)}&limit=${limit}`); return {podcasts:(data.podcasts||[]).map(x=>normalizeArtworkFields(x,{kind:'show'})),source:data.source||'apple-chart',isFallback:false} }
...
export async function fetchPodcastFeed(feedUrl) {
  ...
  const show=normalizeArtworkFields(data.show||{feedUrl},{kind:'show'})
  return {...data,show,episodes:sortEpisodesNewestFirst(data.episodes||[]).map(ep=>normalizeArtworkFields({...show,...ep,podcastId:ep.podcastId||show.podcastId,podcastGuid:ep.podcastGuid||show.podcastGuid,feedUrl:ep.feedUrl||show.feedUrl},{kind:'episode'}))}
}
