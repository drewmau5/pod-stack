...
## Editorial Formulation House design system

[`BRAND_GUIDELINES.md`](./BRAND_GUIDELINES.md) is the source of truth for PodStack's visual, verbal, responsive, accessibility, and interaction design. The Editorial Formulation House identity combines a slim editorial rail, formulation labels, paper layers, catalogue details, curated stack cards, and compact proof-of-product inserts. Brand foundations live in `src/styles/`: exact colour and spacing tokens, Bricolage Grotesque display type, IBM Plex Sans interface type, IBM Plex Mono utility labels, a low-opacity reusable print texture, and a coordinated CSS entrance sequence with a reduced-motion mode.

Podcast covers remain authentic artwork from Apple metadata and publisher RSS feeds; fixed homepage examples use stable local assets and are kept separate from recommendation data. Signed-in Today, My Stack, Discover, onboarding, and profile screens use a deliberately reduced version of the material system so working controls and real podcast content remain primary rather than recreating the homepage collage.
